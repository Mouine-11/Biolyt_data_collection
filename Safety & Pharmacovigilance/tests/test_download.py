...
try:
    with sync_playwright() as p:
        ...
        # Launch with arguments to prevent hangs and run smoothly
        ...
        # We need to make sure the export menu or element is present
        ...
        if export_el:
            ...
            print("Attempting to click 'Export to Excel' and capture download...")
            try:
                with page.expect_download(timeout=60000) as download_info:
                    # Click with force=True since it might be hidden in the menu
                    page.locator(export_selector).click(force=True)
            ...
            print("Export to Excel element NOT found in the DOM. Printing all links:")
            links = page.query_selector_all("a")
            for i, l in enumerate(links[:20]):
                print(f"  {i}: text='{l.inner_text()}' id='{l.get_attribute('id')}'")
