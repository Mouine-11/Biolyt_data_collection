...
Description: Extracts scholarly metadata from the OpenAlex API (focusing on Health Sciences/Medicine),
             ...
             Includes cursor-based resume capabilities and parallel partitioning.
"""
...
class RelationalCSVWriter:
    ...
    def __init__(self, output_dir: Path, enable_relational: bool = False):
        self.output_dir = output_dir
        self.enable_relational = enable_relational
        self.lock = threading.Lock()
        
        # Core Output Files
        self.works_file = output_dir / "openalex_works.csv"
        self.works_fields = [
        ...
        if self.enable_relational:
            self.authors_file = output_dir / "openalex_authors.csv"
            self.institutions_file = output_dir / "openalex_institutions.csv"
            self.sources_file = output_dir / "openalex_sources.csv"
            self.affiliations_file = output_dir / "openalex_affiliations.csv"
...
def fetch_page(cursor: str, api_key: str, filters: str, query: str, per_page: int = 100) -> dict:
    """
    Sends a query page request to OpenAlex works endpoint, handling 
    throttling retries on 429 rate limiting and server issues.
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if r.status_code == 200:
                ...
                logger.error(f"Forbidden (403). Check that your API key is correct and active. Response: {r.text}")
                r.raise_for_status()
            else:
...
def run_partition_worker(
    ...
    while True:
        ...
        try:
            payload = fetch_page(cursor, api_key, filters, query, per_page=per_page)
            ...
            # Polite throttle delay if running without API key
            if not api_key:
                time.sleep(1.0)
            else:
...
def run_downloader(
    ...
    if not api_key:
        logger.warning(
            "No OPENALEX_API_KEY detected. Running in Public Sandbox mode with strict "
            "credit/rate limit controls. Consider adding your API key to the .env file."
        )
    else:
        logger.info("OpenAlex API key found. Authenticated requests active.")
    ...
    writer = RelationalCSVWriter(output_dir, enable_relational=enable_relational)
    tracker = SafeProgressTracker(output_dir / "openalex_progress.json")
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="Literature & Evidence/openalex",
    ...
    parser.add_argument(
        ...
        help="OpenAlex API key (overrides OPENALEX_API_KEY env variable)."
    )
    parser.add_argument(
        "--threads",
        type=int,
