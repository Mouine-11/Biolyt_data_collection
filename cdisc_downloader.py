...
CDISC_DIR = PROJECT_DIR / "Ontologies & Standards" / "CDISC"
...
def discover_nci_standards() -> list:
    """Scrapes the NCI CDISC root directory to find standards folders."""
    logger.info("Discovering CDISC directories from NCI EVS...")
    try:
        html = fetch_url_content(BASE_NCI_URL)
        dirs = []
        for m in re.finditer(r'href=["\']([^"\']+/?)["\']', html):
            link = m.group(1)
            # Filter for subdirectories, ignore parents/Archive/meta directories
            if link.endswith('/') and not link.startswith('/') and link not in ('Archive/', 'schema/', 'xsl/', '/ftp1/'):
                dirs.append(link.rstrip('/'))
        return dirs
    except Exception as e:
        logger.error(f"Failed to parse NCI CDISC directory: {e}")
        return []

def discover_terminology_files(standard_dir: str) -> list:
    """Finds all Terminology or Glossary text files in a standard directory."""
    url = urllib.parse.urljoin(BASE_NCI_URL, f"{standard_dir}/")
    logger.info(f"Scanning directory {standard_dir} at {url}...")
    try:
        html = fetch_url_content(url)
        files = []
        for m in re.finditer(r'href=["\']([^"\']+)["\']', html):
            f = m.group(1)
            # Look for term files ending in .txt, ignore changes/readme/stamps
            if f.endswith('.txt') and not f.startswith('/') and 'Changes' not in f and 'Stamp' not in f and 'ReadMe' not in f:
                files.append(f)
        return files
    except Exception as e:
        logger.error(f"Failed to scan directory {standard_dir}: {e}")
        return []
