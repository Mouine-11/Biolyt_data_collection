...
def extract_info_via_minimax(text: str, api_key: str, base_url: str, model: str) -> dict:
    """
    Sends cleaned text to MiniMax API to dynamically extract common metadata 
    and paper-specific relevant clinical details in JSON format.
    """
    ...
    # Truncate text to fit within typical model context limits safely (approx 25k chars)
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 429:
                logger.warning(f"MiniMax rate limited (429). Retrying in {(attempt + 1) * 5}s...")
            ...
            if "<think>" in content and "</think>" in content:
                ...
                # If only start tag is present, split by it
                parts = content.split("<think>", 1)
                content = parts[0].strip() if len(parts[0].strip()) > 10 else parts[1].strip()
                
            # Clean up markdown code blocks if wrapped by model
            ...
            logger.warning(f"MiniMax API call attempt {attempt+1} failed: {e}")
            if attempt == 2:
                raise e
            time.sleep((attempt + 1) * 2)
            
    raise RuntimeError("Failed to extract information from MiniMax API.")

# ---------------------------------------------------------------------------
# Parallel Worker for Article Processing
# ---------------------------------------------------------------------------

def process_single_article(article_data: dict, api_key: str, base_url: str, model: str) -> dict:
    """
    Worker function to fetch XML, parse clean text, and call MiniMax API.
    """
    ...
    try:
        ...
        # 3. Call MiniMax
        logger.debug(f"Calling MiniMax for {art_id}...")
        llm_data = extract_info_via_minimax(clean_text, api_key, base_url, model)
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="Literature & Evidence/europe_pmc",
        ...
        while True:
            ...
            for attempt in range(5):
                try:
                    ...
                    logger.error(f"EBI API call attempt {attempt+1} failed: {e}")
                    if attempt == 4:
                        raise e
            ...
            for res in results:
                ...
                # De-duplicate metadata
                if art_id in completed_ids:
                    logger.debug(f"  Skipping existing metadata for {art_id}")
                    continue
                ...
                # Queue open-access article for LLM extraction if enabled
                if args.extract_llm and art_id not in completed_llm_ids:
            ...
            if args.extract_llm and llm_queue:
                ...
                with ThreadPoolExecutor(max_workers=args.threads) as executor:
                    futures = {
                        executor.submit(process_single_article, art, api_key, base_url, args.llm_model): art["id"] 
                        for art in llm_queue
                    }
